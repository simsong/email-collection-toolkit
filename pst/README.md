...
Transfers use 64 KiB buffers and temporary files. Hashes are computed from the
...
transfers restart rather than using HTTP range resume. An artifact left without
its receipt must be inspected/moved aside before retrying.
...
arguments use exit 2. The `.download.lock` file prevents simultaneous writers to
one cache. On a crash, check that no downloader is running before removing the
stale lock. Run reports preserve earlier invocation results; the latest report
