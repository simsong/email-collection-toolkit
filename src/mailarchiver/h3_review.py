...
def _annotate_case(case_root: Path, case: ReviewCase) -> ReviewCase:
    ...
    for variant in case.variants:
        raw = (case_root / variant.filename).read_bytes()
        ...
        raw_by_filename[variant.filename] = raw
    ...
    variants = [
        variant.model_copy(
            update={
                ...
                "apple_autosave": _has_header(
                    raw_by_filename[variant.filename], b"x-apple-auto-saved"
                ),
            }
        )
        for variant in case.variants
    ...
    dates = [
        _semantic_header_occurrences(raw_by_filename[variant.filename], b"date")
        for variant in case.variants
    ]
    subjects = [
        _semantic_header_occurrences(raw_by_filename[variant.filename], b"subject")
        for variant in case.variants
    ]
...
def _write_index(output: Path, review: ReviewSet) -> None:
    ...
    (output / "summary.csv").chmod(0o600)
...
def refresh_review_report(output: Path) -> ReviewSet:
    ...
    cases = [_annotate_case(output / case.directory, case) for case in review.cases]
    ...
    for case in cases:
        _write_case_manifest(output / case.directory, case)
