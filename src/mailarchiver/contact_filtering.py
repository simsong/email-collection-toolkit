...
def load_contact_filters(path: Path) -> ContactFilterConfiguration | ContactFilterExtension:
    ...
    if isinstance(contents, dict) and contents.get("mode") == "extend":
        return ContactFilterExtension.model_validate(contents)
    return ContactFilterConfiguration.model_validate(contents)
...
def classify_address(
    ...
    if _matches(local, configuration.bogus_local_part_patterns) or _matches(
        domain, configuration.bogus_domain_patterns
    ):
