...
from .__main__ import IngestOutcome, IngestRequest, run_ingest
    ...
    def _refresh_macos_menu(self) -> None:
        ...
        try:
            from PyObjCTools import (
                AppHelper,  # pylint: disable=import-error,import-outside-toplevel
            )
            from webview.platforms.cocoa import (
                BrowserView,  # pylint: disable=import-error,import-outside-toplevel
            )
        except ImportError:
            return
...
def install_macos_quit_guard(application: PyWebViewApplication) -> None:
    ...
    class ImportAwareDelegate(BrowserView.AppDelegate):
        def applicationShouldTerminate_(self, app):
            ...
            decision = objc.super(ImportAwareDelegate, self).applicationShouldTerminate_(app)
            if decision == appkit.NSTerminateCancel:
                owner._quitting = False
            return decision

    # pywebview replaces this Objective-C delegate class at startup.
    setattr(BrowserView, "AppDelegate", ImportAwareDelegate)
