...
packaged application includes a seed US database with ZCTA, city, state,
...
`make geography-data` refreshes this data. The future **Tools → Update Geo
...
have independent lifecycles. Archive schema changes use a dedicated Flyway
migration history and archive database version; migrations are applied in
