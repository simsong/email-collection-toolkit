...
* `refresh-index` rebuilds the disposable FTS database in a temporary file,
  ...
  legacy charset recovery without changing canonical mail. `review` currently
  queries the committed source-observation log with an optional run filter;
  source and disposition filters are planned. Derived catalog fields and
  canonical locations are created correctly during ingest.
